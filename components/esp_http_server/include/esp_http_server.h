...
#include <lwip/sockets.h>   /* struct ifreq on ESP8266 */
...
#endif // CONFIG_HTTPD_ENABLE_EVENTS || __DOXYGEN__
 ...
 *   - tskNO_AFFINITY does not exist in ESP8266 FreeRTOS (single-core).
 *     Use 0 instead (core_id is ignored on ESP8266 anyway).
 */
...
typedef struct httpd_config {
    ...
    BaseType_t  core_id;            /*!< The core the HTTP server task will run on (ignored on ESP8266 - single core) */
    uint32_t    task_caps;          /*!< Memory capabilities for task stack allocation (ignored on ESP8266) */
