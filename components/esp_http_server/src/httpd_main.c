...
{
    esp_err_t err = esp_event_post(ESP_HTTP_SERVER_EVENT, event_id, (void *)event_data, event_data_size, ESP_HTTP_SERVER_EVENT_POST_TIMEOUT);
    if (err != ESP_OK) {
...
static esp_err_t httpd_server_init(struct httpd_data *hd)
{
    ...
    if (hd->config.if_name && hd->config.if_name->ifr_name[0] != 0) {
        ...
        /* SO_BINDTODEVICE may not be available on ESP8266 lwip */
...
#else
        ESP_LOGW(TAG, LOG_FMT("SO_BINDTODEVICE not supported, interface binding ignored"));
#endif
    }
