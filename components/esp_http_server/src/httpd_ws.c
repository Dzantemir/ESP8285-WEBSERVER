...
esp_err_t httpd_ws_respond_server_handshake(httpd_req_t *req, const char *supported_subprotocol)
{
    ...
    /* Generate SHA-1 hash using mbedtls (ESP8266 compatible) */
    int ret = mbedtls_sha1_ret((const unsigned char *)server_raw_text, strlen(server_raw_text), server_key_hash);
    if (ret != 0) {
        ESP_LOGE(TAG, "Failed to calculate SHA-1 hash (mbedtls error: -0x%04x)", -ret);
        return ESP_FAIL;
    }
